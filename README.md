...
## 📧 Internal SMTP Setup for Grafana Alerts

Since all servers in this project are **offline** and joined to the **Active Directory domain**, using external notification services like Gmail, Slack, or Telegram is **not feasible**.  

To ensure email alerts can be received and tested internally, I installed an **SMTP service directly on the Windows Domain Controller (DC)**. This approach provides an **internal mail server** within the domain that can handle alerts generated by Grafana for all monitored servers.  

By configuring Grafana to use this internal SMTP server, the system can send alerts for critical metrics such as CPU usage, memory usage, and disk space without relying on any external internet access. This setup guarantees that **all notifications remain within the offline network**, maintaining security and reliability.

![Installation of SMTP service on PDC Succeeded](images/smtp-install-dc.png)

The following **Prometheus alert queries** were configured in Grafana:
...
100 * (1 - (windows_os_physical_memory_free_bytes / windows_os_virtual_memory_bytes))

# C Drive Usage Windows
100 * (1 - (windows_logical_disk_free_bytes{volume="C:"} / windows_logical_disk_size_bytes{volume="C:"} * 100))


# SMTP Alert Mail Drop Location (Windows DC)is C:\inetpub\mailroot\Drop

```
Using this configuration, Grafana automatically sends email alerts to the internal SMTP server whenever any of the above metrics exceed their defined thresholds, allowing the monitoring team to react promptly even in a completely offline environment.

### ⚠️ Grafana Alerts Overview

The following alert rules were configured in Grafana to monitor critical system metrics across all Linux and Windows servers.  
These alerts provide **real-time visibility** into CPU usage, memory utilization, and disk space, ensuring proactive response to potential issues.  

- Alerts are **threshold-based**, firing when metrics exceed predefined safe limits.
- Notifications are sent directly to the **internal SMTP server** on the Windows Domain Controller, keeping the system fully offline and secure.
- Each alert is carefully named and grouped for clarity, making it easy for administrators to identify and respond to issues quickly.

### 🔹 Prometheus Alert Rules in Grafana

![Grafana Alert Rules](images/alerts.png)


### 📧 Grafana SMTP Test

After configuring the SMTP service on the Windows Domain Controller, a **test email was successfully sent** from Grafana to the internal mail server.  
This confirms that all alerts configured in Grafana can reliably trigger email notifications in our offline environment.

![Grafana SMTP Test](images/alert-test.png)

### ✉️ Sample Alert Emails from Grafana

The following are **two examples of alert emails** successfully sent by Grafana via the internal SMTP server.  
These emails confirm that the configured alerts (CPU, memory, and disk usage) are working as expected in our offline, domain-joined environment.
...
## 👩‍💻 Author
**Elham Hassan**  
🔧 DevOps Enthusiast | System Admin | Automation Engineer | Cloud/DevOps Engineer
